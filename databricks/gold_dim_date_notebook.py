...
df_joined = df_src.join(df_sink, df_src['date_id'] == df_sink['date_id'], 'left')\
...
df_filter_existing = df_joined.filter(col('dim_date_key').isNotNull())
df_filter_existing.display()

# COMMAND ----------

# DBTITLE 1,filtering new records
df_filter_new = df_joined.filter(col('dim_date_key').isNull())\
    .select(col('date_id'))
...
if incremental_flag == '0':
  ...
  df_max_dim_date_key = spark.sql("""
    SELECT MAX(dim_date_key)
    FROM cars_catalog.gold.dim_date;
  """)
  max_dim_date_key = df_max_dim_date_key.collect()[0][0]

print(max_dim_date_key)

# COMMAND ----------

# DBTITLE 1,insert surrogate keys to new records
# adding 1 because montonically_increasing_id() function starts from 0
df_filter_new = df_filter_new.withColumn('dim_date_key', max_dim_date_key + monotonically_increasing_id() + 1)
df_filter_new.display()
...
if spark.catalog.tableExists('cars_catalog.gold.dim_date'):
  ...
  delta_table = DeltaTable.forPath(spark, 'abfss://gold@adlscardataprojectdev.dfs.core.windows.net/dim_date')
  ...
  df_final.write.format('delta')\
    .mode('overwrite')\
    .option('path', 'abfss://gold@adlscardataprojectdev.dfs.core.windows.net/dim_date')\
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM cars_catalog.gold.dim_date;
